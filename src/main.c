...
void inicio() {
    ...
    for (int i = 0; i < MAX_BALAS; i++) balas[i].ativo = 0; // Inativa todas as balas
    for (int i = 0; i < MAX_ASTEROIDES; i++) asteroides[i].ativo = 0; // Inativa todos os asteroides
...
void movimento(char direcao) {
    if (direcao == 'L' && nave.x > 0) nave.x--; // Move para a esquerda
    if (direcao == 'R' && nave.x < WIDTH - 5) nave.x++; // Move para a direita
}

// Função para atirar uma bala
void atirar() {
    for (int i = 0; i < MAX_BALAS; i++) {
        if (!balas[i].ativo) { // Procura uma bala inativa
...
void movertiros() {
    for (int i = 0; i < MAX_BALAS; i++) {
        if (balas[i].ativo) { // Verifica se a bala está ativa
            balas[i].y--;     // Move a bala para cima
            if (balas[i].y < 0) balas[i].ativo = 0; // Desativa a bala ao sair da tela
        }
    }
}
...
void gerarasteroides() {
    for (int i = 0; i < MAX_ASTEROIDES; i++) {
        if (!asteroides[i].ativo) { // Procura um asteroide inativo
